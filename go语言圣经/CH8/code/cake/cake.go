...
type Shop struct {
	...
	BakeStdDev     time.Duration // bake的标准偏差
	BakeBuf        int           // baking和icing的缓冲区
	NumIcers       int           // 做icing厨师的数量
	IceTime        time.Duration // time to ice one cake
	IceStdDev      time.Duration // ice的标准偏差
	IceBuf         int           // icing和inscribing的缓冲区
	InscribeTime   time.Duration // time to inscribe one cake
	InscribeStdDev time.Duration // inscribing的标准偏差
}
