...
`const 名字 [类型] = 值`
常量表达式的值在`编译期`计算，不是运行期；常量的潜在类型都是基础类型：`boolean`、`string`或数字, 常量的值不可修改。
所有常量的运算都在编译期完成，常量的算数运算、逻辑运算和比较运算的结果也是常量；
常量声明如果没有显示指明类型，那么将从右边的表达式推断类型；

如果批量声明的变量，除了第一个外其他常量右边的初始表达式都可以省略，如果省略则表示使用前面常量的表达式写法；
```
const (
	a = 1
	b
...
`iota`常量生成器用于生成一组以相似规则初始化的常量；
第一个声明的常量所在的行，iota会被设置为0，然后每一个有常量声明的行加一
示例： `code/netflag/main.go`

### 3.6.2 无类型常量

只有常量是可以无类型的，无类型的数字常量会提供比基础类型更高精度的算术运算；
通过延迟明确常量的具体类型，无类型的常量不仅可以提供更高的运算精度，
而且可以直接用于更多的表达式而不需要显式的类型转换。比如：
```
math包中的Pi被设置为常量(math/const.go)，所以math.Pi可以直接用于任意需要浮点数或者复数的地方
